...
public class LocationController {
	...
	public void insert_data() {
		        	...
		        	String day = currentDate.format(formatter);
		        		for(int k=0; k<24; k++) {
			                ...
			                if (batchData.size() >= 1000) { // 예를 들어, 1000개마다 배치 실행
			                	locationDao.insertHour(batchData);
		...
		try {
			...
			for(Map<String, Object> map : list) {
		        ...
		        while (!currentDate.isAfter(endDate)) {
		    		...
		    		// 배치 처리
	                if (batchData.size() >= 1000) { // 예를 들어, 1000개마다 배치 실행
	                	locationDao.insertDay(batchData);
	                    batchData.clear(); // 배치 데이터 초기화
	                }
		...
		try {
			// 월 데이터
			List<Map<String, Object>> monthList = locationDao.getYearMonth();
			for(Map<String, Object> month : monthList) {
                ...
                if (batchData.size() >= 500) { // 예를 들어, 1000개마다 배치 실행
                	locationDao.insertMonth(batchData);
                    batchData.clear(); // 배치 데이터 초기화
                }
			}
		} catch (Exception e) {
		...
		try {
			// 월 데이터
			List<Map<String, Object>> yearList = locationDao.getYear();
			for(Map<String, Object> year : yearList) {
                ...
                if (batchData.size() >= 100) { // 예를 들어, 1000개마다 배치 실행
                	locationDao.insertYear(batchData);
                    batchData.clear(); // 배치 데이터 초기화
                }
			}
		} catch (Exception e) {
